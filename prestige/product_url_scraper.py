...
class Database:
    ...
    def batch_insert_product_urls(self, urls: list) -> tuple:
        ...
        try:
            ...
            for url in urls:
                try:
                    # Try to insert new URL
                    cursor.execute('''
                        INSERT INTO products (product_url, url_discovered_at)
                        VALUES (?, CURRENT_TIMESTAMP)
                    ''', (url,))
                    inserted_count += 1
                except sqlite3.IntegrityError:
                    # URL already exists, update last_checked timestamp
                    cursor.execute('''
...
class URLScraper:
    ...
    async def scrape_and_save_urls(self, category_url: str) -> dict:
        ...
        try:
            ...
            inserted_count, updated_count = self.db.batch_insert_product_urls(urls)

            logger.info(f"✅ Inserted {inserted_count} new product URLs")
            if updated_count > 0:
                logger.info(f"🔄 Updated {updated_count} existing URLs")

            return {'inserted': inserted_count, 'updated': updated_count, 'total': len(urls)}
        except Exception as e:
